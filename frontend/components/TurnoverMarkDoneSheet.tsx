...
import { turnovers as turnoversApi, uploads, ApiError } from '@/lib/api';
import type { Turnover, IncidentPriority } from '@/lib/api';
import type { Translations } from '@/i18n/translations';

interface TurnoverMarkDoneSheetProps {
  turnover: Turnover;
  t: Translations;
  onClose: () => void;
...
export function TurnoverMarkDoneSheet({
  turnover,
  t,
  onClose,
  onSuccess,
}: TurnoverMarkDoneSheetProps) {
  ...
  // Free blob URLs when sheet unmounts
  useEffect(() => {
    return () => {
  ...
  // Display fields — handle null bookings
  ...
  const bookingRef =
    turnover.toBooking?.bookingRef ??
    turnover.fromBooking?.bookingRef ??
    '';
  ...
  async function handleAllGood() {
    ...
    setError('');
    try {
      const res = await turnoversApi.markDone(turnover.id, { allGood: true });
      onSuccess(res.turnover);
    } catch (err) {
  ...
  async function handleSubmitIssue() {
    ...
    setError('');
    try {
      const res = await turnoversApi.markDone(turnover.id, {
        allGood: false,
        priority,
      ...
      onSuccess(res.turnover);
      // Auto-navigate to the newly created incident if one was made
      if (res.incidentId) {
    ...
    >
      ...
      >
        ...
        <div className="mb-4 bg-surface rounded-xl p-3">
          <p className="text-sm font-semibold text-ink truncate">{accommodationName}</p>
          {bookingRef && (
            <p className="text-xs text-ink-muted mt-0.5 truncate">{bookingRef}</p>
          )}
        </div>
