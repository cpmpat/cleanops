        ...
        <div className="bg-white border border-surface-border rounded-2xl p-5 space-y-3">
          ...
          {incident.cleaning && (
            <MetaRow
              ...
              value={
                <span>
                  {incident.cleaning.accommodationName} —{' '}
                  {new Date(incident.cleaning.timeSlot).toLocaleString()}
                  {incident.cleaning.booking?.bookingRef && (
                    <span className="text-ink-faint">
                      {' '}(ref {incident.cleaning.booking.bookingRef})
                    </span>
                  )}
                </span>
              }
          ...
          {!incident.cleaning && incident.booking && (
            <MetaRow
              icon={<Calendar size={14} />}
              label={t.incidents.detail.cleaning ?? 'Booking'}
              value={
                <span>
                  {incident.booking.accommodationName} —{' '}
                  {new Date(incident.booking.checkInTime).toLocaleDateString()}
                  <span className="text-ink-faint">
                    {' '}(ref {incident.booking.bookingRef})
                  </span>
                </span>
              }
            />
